...
public class Connection extends Thread {
    ...
    public void handleJoinRoom(String inMessage) throws IOException {
        if (inMessage.equals("create")) {
            createRoom();
            transmission.sendObject(new Response(StatusResponse.ROOM, room.getToken() + "," + room.getClient1().getUsername() + ",," + 0 + "," + 1));
        } else {
            if (inMessage.equals("public")) {
                ...
                if (!isCheck) {
                    ...
                    transmission.sendObject(new Response(StatusResponse.ROOM, room.getToken() + "," + room.getClient1().getUsername() + "," + room.getClient2().getUsername() + "," + 0 + "," + 0));
	...
	public void handleExitRoom() throws IOException {
		if (this.id == room.getClient1().id) {
			if (room.getClient2() != null) {
				room.getClient2().transmission.sendObject(new Response(StatusResponse.UPDATE_ROOM, room.getToken() + "," + room.getClient2().getUsername() + ",," + 0 + "," + 1));
			...
			room.getClient1().transmission.sendObject(new Response(StatusResponse.UPDATE_ROOM, room.getToken() + "," + room.getClient1().getUsername() + ",," + 0 + "," + 1));
			room.setClient2(null);
			room = null;
		}
	}
    ...
    public void handleDisConnect() throws IOException {
        if (null != room) {
            if (room.getClient1().id == id) {
                if (null != p) {
                    p.recvMess(this, new Request("lose", ""));
                    p.interrupt();
                    p = null;
                }
                room.setClient1(room.getClient2());
